...
SafeRational(num::T2, den::T2) where {T1<:SafeSigned, T2<:SafeSigned} = T1(num)//T1(den)
SafeRational(num::T2, den::T2) where {T1<:Signed, T2<:Signed} = safeint(T1(num))//safeint(T1(den))

SafeRational(num::T2, den::T2) where {T1<:SafeSigned, T2<:Signed} = T1(num)//T1(den)
SafeRational(num::T2, den::T2) where {T1<:Signed, T2<:SafeSigned} = safeint(T1(num))//safeint(T1(den))

SafeRational(num::T2, den::T3) where {T1<:SafeSigned, T2, T3} = T1(num)//T1(den)
SafeRational(num::T2, den::T3) where {T1<:Signed, T2, T3} = safeint(T1(num))//safeint(T1(den))
